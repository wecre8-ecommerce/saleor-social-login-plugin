...
class Provider:
    ...
    def fetch_profile(self, access_token):
        ...
        raise ValidationError(
            message="An error occurred while requesting {}: {}".format(
    ...
    def get_email(self, access_token, **kwargs):
        ...
        if email is None:
            raise ValidationError(
                "User does not allow email access to your application"
            )
...
class Facebook(Provider):
    ...
    def validate_userinfo_url(self, _for):
        return self.urls["userinfo"] + "?fields=email,first_name,last_name"
...
class Apple(Provider):
    ...
    def get_email(self, **kwargs):
        ...
        This introduces a vulnerability which should be solved
        if the ID token is crafted # FIXME

        """
        payload = self.decode_token(
            token=kwargs.get("id_token") or kwargs.get("access_token")
        )
    ...
    def fetch_profile(self, access_token):
        email, _, _ = self.get_email(kwargs={"id_token": access_token})
