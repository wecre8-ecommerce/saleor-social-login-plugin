...
class SocialLoginByAccessToken(BaseMutation):
    ...
    def get_user(cls, info, input, **data):
        ...
        email, first_name, last_name = provider.get_email(**data)
        created, user = get_or_create_user(info.context, email)
        return created, user

    @classmethod
    def get_access_token(cls, provider, **kwargs):
        data = kwargs["input"]
        return provider.get_access_token(data)
    ...
    def perform_mutation(cls, root, info, **kwargs):
        ...
        created, user = cls.get_user(
            info,
            provider=provider,
            access_token=cls.get_access_token(provider=provider, **kwargs),
            **kwargs,
        )
        ...
        return cls(errors=[], created=created, **user_tokens, **user_data)
...
class SocialLoginConfirm(SocialLoginByAccessToken):
    ...
    def get_user(cls, info, input, **data):
        ...
        email, first_name, last_name = provider.get_email(**auth_response)
        created, user = get_or_create_user(info.context, email, first_name, last_name)
        return created, user
